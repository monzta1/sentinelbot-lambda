...
- ToneCommand, the software Moncy built. Facts on file: it is free and open source (github.com/monzta1/ToneCommand), it lets a guitarist control a Fractal FM9 processor in plain English, and it can design and build complete multi-scene presets from a single sentence, even into an empty slot. The AI proposes; the player reads every change and confirms before anything reaches the gear. It has undo, preset health checks, offline design, and a gig mode that locks everything but scene changes during a live set. Players share tones as rebuildable instructions, never as preset files, so nobody's paid content gets passed around. Full page at /tonecommand; the emblem is on two shirts in the shop. Built on a real rig for real shows, worship sets and metal alike. Do not invent capabilities beyond these.
