...
## v1.9.4 - May 2026
- Tightened the system prompt's scope rules. Two real-user questions ("who is the drummer" and "why is the music so bad") opened with "That is outside my watch" before walking it back into a correct answer. Both questions are on-topic about the music. The previous prompt listed off-topic categories (sports, cooking, politics) but did not list on-topic categories explicitly, so the model leaned on the deflect-and-bridge pattern even for clearly in-scope queries.
- New structure: an ANSWER DIRECTLY list (music critique, players, gear, production, mission, faith, AI-in-music) and a narrower DEFLECT list (truly unrelated trivia). The bridge pattern is now constrained with explicit valid/invalid examples ("Jupiter's moons" valid bridge to Galilean; "who is the drummer" never deflects, drums are EZdrummer).
- Added a BANNED OPENINGS section to the prompt. Live testing after the first scope-rule revision showed the model still leading with "That is outside my watch" on the drummer question. The banned-openings list explicitly forbids that phrase (and rephrasings) as the first sentence for any question touching the music, musicians, gear, production, mission, faith, or AI-in-music.
- Added deterministic cached answers for drummer questions ("who is the drummer", "who plays drums", "are the drums real"). Routes through `findCachedAnswer` so these short-circuit the LLM entirely and avoid the deflection class of bugs by construction.
- Added a cached confident defense for music critique ("bad", "sucks", "terrible", "awful", "weak", "garbage", "trash", "boring", "mid" combined with "music"/"song"/"tracks"/"album"). Live testing after the prompt rewrite showed the LLM was still opening with "That's outside my watch" on "why is the music so bad" even with the explicit BANNED OPENINGS rule. A cached deterministic answer is the reliable fix; the response pushes back on the standard and points to the documented production pipeline.
- Verified end-to-end against the four test cases: drummer (cached EZdrummer answer), music-bad critique (cached defense), variant phrasings (cache routing catches them), and sports-trivia control (still correctly deflects with bridge pattern).
