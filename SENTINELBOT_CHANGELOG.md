...
- "what is tonecommand" no longer gets "outside my watch". ToneCommand is
  ...
  list entirely, so the model filed it with sports scores. Added to the
  ANSWER DIRECTLY list with a grounded fact block (free, open source, FM9
  in plain English, confirm-before-send, undo, health checks, gig mode,
  recipe sharing, /tonecommand page, shop shirts) and an explicit
  instruction not to invent capabilities beyond those on file.
