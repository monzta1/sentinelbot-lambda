...
## v1.10.0 - May 2026
- SentinelBot now retrieves band-voice and announcement context from the social stream alongside the existing song-lifecycle context from the event stream. Sensitivity-gated; do-not-answer rows are filtered before reaching the model. Dark-launched behind SOCIAL_CONTEXT_ENABLED; see docs/phase-3-rollout-plan.md.
- `lookupSocialContext(question, options)` classifies the user's question against the 16-tag vocabulary, loads pk = "socialstream" rows into an in-memory 5-minute cache (band voice does not change minute to minute), filters out do-not-answer rows unconditionally, filters out routed rows unless the question matches the routed allowlist (stance / opinion / gatekeep / authentic / AI-in-music phrasings), and ranks remaining rows by tag-match plus recency. Returns up to SOCIAL_CONTEXT_MAX_ROWS (default 5).
- `buildSocialContextSystemBlock(rows)` assembles the chosen rows into a single "## Band Voice and Recent Posts" system block injected after the Signal Room block and before any per-song extraContext. Each row line carries the timestamp (publishedAtIso if known else capturedAt), tags, and the LLM summary (preferred) or a 400-char body preview.
- Token soft-limit truncation: if the assembled blocks would exceed ANTHROPIC_INPUT_TOKEN_SOFT_LIMIT (default 12000), the social-context block is trimmed line-by-line, or dropped if there's not enough headroom for at least three lines. The other system blocks are never touched by this trimmer.
- Post-response guardrail: any 40-char verbatim overlap between the model's response and a do-not-answer post body replaces the response with the canonical Contact-page reference. The primary defense is filtering those rows out in the ranker; this is paranoid defense in depth.
- Performance: p95 retrieval cost on a synthetic 1000-row table is well under the 200ms budget after the cache warms up (verified by test). First warm-up pays one Scan against pk = "socialstream"; subsequent calls hit the in-memory cache.
- Hot-path performance budget documented in docs/phase-3-rollout-plan.md alongside cost notes, the rollout sequence (staging 24-hour bake then production env-var flip), and rollback (single env-var change, no redeploy).
- 9 new tests under tools/shield-cli/tests/run-social-context-tests.js cover: AI-disclosure retrieval, do-not-answer filtering in every code path including via the public lookup entry, routed allowlist gate, question-tag classifier, allowlist recognition, p95 latency on 1000 rows, byte-identical prompt assembly when the flag is off, do-not-answer overlap guard, and token-soft-limit truncation. 233 + 19 + 10 + 9 + 70 = 341 tests across all suites; all green.
- `sentinelbot-scraper-staging/prompt-assembly.js` and `sentinelbot-prompt.txt` updated to document the new "BAND VOICE AND RECENT POSTS" runtime section so staging composition tools and the runtime stay in sync on section names.

## v1.10.0-rc.1 - May 2026
- Added `sentinelbot-social-ingest/`, the real-time ingest Lambda for the socialstream partition. Two modes selectable by `SOCIAL_INGEST_MODE`: `poll` (default; EventBridge fires the handler every 15 minutes which then pulls `/{page-id}/posts?since=<cursor>` from the Graph API) and `webhook` (API Gateway routes Meta Page webhook deliveries here after the Lambda verifies the `X-Hub-Signature-256` HMAC against `META_APP_SECRET`). Both modes funnel into the same `runRecaptureForPost` decision logic the backfill uses, so a row produced by real-time ingest is byte-identical to a row produced offline.
- Extracted shared logic into `lib/social-ingest-core.js`. `decideAction`, the ID-resolution chain (permalink ID then stabilityHash then contentHash), the transform pipeline (normalize, tag classification, sensitivity classification, tag-vocabulary validation), `buildItem`, `enrichWithAnthropic`, and the `runRecaptureForPost` / `runRecaptureBatch` orchestrators all live there. `scripts/backfill-social-stream.js` now imports from the lib; behavior is unchanged (19 social-stream tests still pass).
- Cursor management: rows under `pk = "socialstream-cursor"`, `sk = "facebook"`, attributes `lastSeenCreatedTime`, `lastRunAt`, `lastRunStatus`. On first run with no cursor, initializes from the newest `publishedAtIso` (falling back to `capturedAt`) across existing socialstream rows. Most Phase 1 rows have `null publishedAtIso`, so the cursor lands on the most recent capturedAt; the first poll then catches the backlog of posts published between Phase 1 ingest and now in a single paginated pass.
- Graph API failure handling per the brief: 401/403 throws `AuthFailure` and the cursor never advances (`lastRunStatus = "auth-failed"`), so a flaky Page access token can't silently lose posts. Other 4xx fail loudly without retry. 429 and 5xx retry up to 5 times with exponential backoff. Webhook signature verification is non-negotiable; unsigned and badly-signed requests get HTTP 401, no further processing.
- 10 new tests under `tools/shield-cli/tests/run-social-ingest-tests.js` cover cursor initialization from empty state, cursor advancement on success, auth failure leaving the cursor untouched, Graph API rate-limit retry, webhook signature verification (valid / invalid / missing), and the `decideAction` integration in poll mode for duplicate (skipped-unchanged) and edited posts. All deps are injected so the suite runs with zero AWS / Meta / Anthropic calls.
- `DRY_RUN=true` exercises the full pipeline against the real Graph API in poll mode but skips DynamoDB writes and LLM enrichment. `FORCE_SINCE=<ISO>` overrides the cursor for one invocation (useful for replaying a window).
- Deployment is the user's job. `docs/phase-2-deployment-checklist.md` lists the exact AWS console / CLI / Meta Business Suite steps in order. `sentinelbot-social-ingest/deploy.sh` is committed but not executed in this task. The Lambda is dormant until the user runs the checklist.
- Phase 1.5 recapture was not exercised; the 152 truncated Phase 1 rows are expected to backfill organically via real-time ingest or a future explicit recapture task.

## v1.9.7 - May 2026
- Added re-capture mode to the social-stream backfill so previously truncated Facebook posts can be upgraded to full bodies and re-enriched without disturbing already-complete rows. Opt in via `SOCIAL_RECAPTURE=true`. The default Phase 1 behavior (insert-or-overwrite by sk) is preserved when the flag is not set.
- Decision logic per row: `insert` for posts not in DynamoDB, `upgraded-from-truncated` when an existing truncated row is now full, `edited` when an existing full row's body changed, `refreshed-truncated` when both are truncated but the new body is longer, `skipped-unchanged` otherwise. LLM enrichment runs only on insert / upgrade / edit when the new body is full; skip and refresh paths never call the LLM. Re-captured rows get a `lastRecapturedAt` attribute.
- Cross-capture lookup is the trickier piece because the original `contentHash` keyed on the first 500 chars of the body, so a truncated row and its full counterpart have different `socialId` values. Added a `stabilityHash` (first 80 chars) as a secondary key. `loadExistingByStableId` now indexes each existing row under socialId, stabilityHash of its stored body, and any FB numeric ID extractable from `sourceUrl`. The orchestrator tries permalink ID -> stabilityHash -> contentHash in order. `buildItem` also preserves the existing row's `socialId`, `stableIdSource`, and `captureIndex` so an upgrade writes back to the same sk instead of creating a duplicate.
- Snapshot helper: `npm run snapshot:social-stream` writes the current state of `pk = "socialstream"` (counts by sensitivity, tag, truncation, plus per-row socialId + textTruncated + hasSummary) to `data/social/snapshots/social-stream-<ISO>.json`. Verify script picks up `SOCIAL_VERIFY_BASELINE=<path>` and prints a delta block (inserted / truncatedToFull / stillTruncated / summaryAdded / leftAlone / baselineOnly).
- Logging: per-row events log socialId and action only; `do-not-answer` rows never have their body content echoed.
- New env var `SOCIAL_EXISTING_FIXTURE_FILE` lets `DRY_RUN=true` mode load synthetic existing rows from a file (instead of scanning DynamoDB) so the five decision branches can be exercised offline with zero AWS or LLM calls. The brief's acceptance dry-run is now reproducible against the fixtures at `/tmp/recapture-new.json` + `/tmp/recapture-existing.json`.
- 6 new tests cover `decideAction` across all branches plus the four behavioral guarantees (preserves capturedAt/traceId, LLM fires on upgrade, no LLM on unchanged, fresh insert on previously-missing post, absent-from-input rows left untouched). 19 total social-stream tests pass; full chain still green.
- `.gitignore` updated to ignore `data/social/` (which transitively covers `data/social/snapshots/`). The path was not previously ignored despite the brief assuming it was.

## v1.9.6 - May 2026
- Phase 1 of the social-post archive: added `scripts/backfill-social-stream.js` and `scripts/verify-social-stream.js` plus two npm aliases (`backfill:social-stream`, `verify:social-stream`). The backfill reads a captured Facebook DOM-scrape JSON file (default `data/social/shieldbearer-facebook-posts.json`, override via `SOCIAL_INPUT_FILE`) and writes rows under `pk = "socialstream"` in the existing `shieldbearer-sentinel-logs` table. The Facebook Page access token has been flaky, so the browser-DOM capture path is the Phase 1 entry. Phase 2 will replace it with a webhook receiver that hits the same `buildItem` transform; PHASE 2 HOOK comments mark the seams.
- Strict partition isolation: the script never writes to `pk = "eventstream"`. The new social stream is its own partition so the bot's existing event-stream consumers and song lifecycle code remain untouched. The bot integration to read from `socialstream` is a separate task in the Lambda.
- Sort key is `<sourceType>#<capturedAt>#<stableId>` so a single capture clusters together. `stableId` prefers a real Facebook numeric ID extracted from the permalink (`/posts/<id>`, `/videos/<id>`, `/reel/<id>`, `story_fbid=<id>`); when none is present, falls back to a deterministic 16-hex sha256 over `sourceType|normalizedText[:500]`. Re-runs overwrite cleanly; `capturedAt` and `traceId` are preserved from any existing record so historical metadata does not drift.
- Each row records `sourceType`, `sourceUrl`, `capturedAt`, `ingestedAt`, `publishedAtRaw` (the FB DOM timestamp label, e.g. "2d"), `publishedAtIso: null` (DOM scrape timestamps are unreliable), `captureIndex`, `textTruncated`, and a `payload` block with `{message, summary, tags, sensitivity, raw, truncated}`. Tags are validated against a fixed vocabulary of 16 values; unknown tags are dropped.
- Sensitivity rules per the task spec: `do-not-answer` when the post matches `/\b(IRGC|Iran|Iranian|Israel|Hamas|Hezbollah|ballistic missile)\b/i`, `routed` when tags include `politics` or `gatekeeping`, `public` otherwise. Truncated posts skip LLM summarization (body too partial to be worth the call) and fall back to keyword-based tagging.
- Anthropic enrichment is optional and honors `ANTHROPIC_API_KEY` and `ANTHROPIC_FALLBACK_MODEL` exactly as `index.js` does. `callAnthropic` itself is not exported from `index.js`, so the script duplicates the minimal POST logic locally with a tight system prompt that asks for JSON only (`{summary, tags}`). On any failure the script falls back to keyword tagging silently.
- CLI flags: `DRY_RUN=true` (no AWS or LLM writes), `LIMIT=N` (safety cap), `SKIP_LLM=true` (skip enrichment), `SOCIAL_REPLACE_EXISTING=true` (force full overwrite of all attributes). Progress is logged every 25 posts.
- Verify script reports counts by `sourceType`, `sensitivity`, and each tag; counts `textTruncated` true vs false; prints the 10 most recent captures by `capturedAt`; counts items missing `payload.summary` so we know how many need re-enrichment when a better capture method lands.
- 13 new unit tests under `tools/shield-cli/tests/run-social-stream-tests.js` cover permalink ID extraction (posts/videos/reel/story_fbid patterns), content-hash determinism, whitespace tolerance, truncation flag handling, sensitivity classification (do-not-answer/routed/public), tag vocabulary validation (unknown drops, casing, fallback to uncategorized), idempotent preservation of `capturedAt` and `traceId` across re-runs, sk format, and file-not-found / malformed-JSON / missing-posts-array error paths. Wired into `npm test:run`.
- Dry-run smoke test against a 5-post synthetic fixture verified the expected sensitivity split (3 public, 1 routed, 1 do-not-answer) and tag distribution.
